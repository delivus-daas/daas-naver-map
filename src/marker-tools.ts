...
// import MarkerOverlappingRecognizer from "./MarkerOverlappingRecognizer";
import $ from "jquery";
 ...
 */
class MarkerClustering extends window.naver.maps.OverlayView {
  // 기본 값입니다.
  DEFAULT_OPTIONS = {
    // 클러스터 마커를 올릴 지도입니다.
    map: null,
   ...
   */
  public setOptions(newOptions: any | string, value: any) {
    var _this = this;
    ...
    try {
      if (typeof newOptions === "string") {
        ...
        window.naver.maps.Util.forEach(newOptions, function (
          value: string,
          key: string
        ) {
          if (key !== "map") {
   ...
   * @return {Any} 옵션
   */
  public getOptions(key: number | string) {
    var _this = this,
      options: any = {};
   ...
   */
  public setMinClusterSize(size: number) {
   ...
   */
  setIndexGenerator(indexGenerator: number) {
   ...
   */
  setDisableClickZoom(flag: boolean) {
   ...
   */
  setAverageCenter(averageCenter: number) {
   ...
   * @return {Cluster} 클러스터
   */
  _getClosestCluster(position: any) {
...
class Cluster {
   ...
   * @private
   */
  _calcAverageCenter(markers: any[]) {
